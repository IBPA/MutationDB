...
def cross_validation(X,Y,setting):
    ...
    for train, test in LeaveOneOut().split(X, Y):
      # over sampling the positive samples
        X_train = X[train,:]
        Y_train = Y[train]
        positive_index = np.where(Y_train==1)[0]
        count = sum(positive_index)
        total = X_train.shape[0]
        while count< total/sampling_ratio:
	    X_train = np.concatenate((X_train,X_train[positive_index,]))
	    Y_train = np.concatenate((Y_train,Y_train[positive_index,]))
	    count = sum(Y_train==1)
            total = X_train.shape[0]

      #over sampling is done
        model.fit(X_train,Y_train,epochs=20,verbose=0, callbacks =callbacks)
...
out_path = "../../out_data/ANN_prediction_oversampling_"+str(sampling_ratio)+"/gene_id_"+str(gene_id) +".csv"
